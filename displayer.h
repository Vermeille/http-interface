...
class Form {
    ...
    Form(const std::string& method, const std::string& action) {
        content_ = "<form method=\"" + method + "\" action=\"" + action + "\">";
    }

    Form& Input(const std::string& name, const std::string& type) {
        content_ += "<input name=\"" + name + "\" type=\"" + type + "\" />";
        return *this;
    }

    Form& Number(const std::string& name) { return Input(name, "number"); }
    Form& Text(const std::string& name) { return Input(name, "text"); }
    Form& Submit(const std::string& name = "Submit") { return Input(name, "submit"); }
    std::string Get() { return content_ + "</form>"; }
};
